...
game's compact stash shape, on the expedition's own ``AFK Farm`` stash (gear)
or in ``AFK Materials`` (native stackables).  Only temporary files are touched.
"""
...
class VaultIngestTests(unittest.TestCase):
    ...
    def test_deleted_stash_is_not_recreated_by_retry_or_new_expedition_layout(self):
        ...
        store = editor.vault_store()
        survivor = store.list_all_available_items(collection=farm_id)[1]
        preview = store.preview_storage_deletion(farm_id, page_index=0)
        store.delete_storage(farm_id, page_index=0, preview_token=preview["previewToken"])
        retry = self._ingest([GEAR, MATERIAL])
        self.assertEqual((retry["deposited"], retry["duplicate"]), (0, 2))
        ensure = editor.op_vault_layout({"action": "ensure", "collectionId": farm_id})
        self.assertEqual([p["pageIndex"] for p in ensure["stashes"]], [1])
        third = editor.op_vault_ingest({"expedition_id": "exp_three", "records": [GEAR]})
        self.assertEqual(third["collections"]["farm"]["pageIndex"], 2)
        self.assertEqual([p.page_index for p in store.list_stash_pages(farm_id)], [1, 2])
        self.assertEqual(store.get_item(survivor.id), survivor)
    ...
    def test_two_records_deposit_once_and_repeat_as_duplicates(self):
        ...
        self.assertEqual(gear.collection_name, "AFK Farm")
        ...
        # One page per expedition, named after it and its date; layout assigned.
        store = editor.vault_store()
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            {
                "farm": {
                    "id": gear.collection_id, "name": "AFK Farm",
                    "pageIndex": 0, "pageName": PAGE_NAME,
                },
    ...
    def test_unique_gear_gets_m_and_each_expedition_gets_its_own_page(self):
        self._ingest([GEAR])
        unique = spool_record(
        ...
        self.assertEqual(
            ...
            ("AFK Farm", 1, "exp_two · 2026-09-17 · Act 3 · 2h"),
        )
        rows = self._rows_by_label()
        crown = rows["Crown"]
        self.assertEqual(
        ...
        self.assertEqual((crown.page_index, crown.layout_x, crown.layout_y), (1, 0, 0))
        cannon = rows["Brute's Hand Cannon"]
        self.assertEqual((cannon.page_index, cannon.layout_x, cannon.layout_y), (0, 0, 0))
        store = editor.vault_store()
        self.assertEqual(
            [page.name for page in store.list_stash_pages(farm["id"])],
            [PAGE_NAME, "exp_two · 2026-09-17 · Act 3 · 2h"],
        )
        ...
        # A later batch of the first expedition lands back on its own page.
        more = self._ingest([spool_record(9, 8, "Belt", {"n": 2.0, "b": 2.0, "a": 5.0, "j": 0, "c": 0.0})])
        self.assertEqual(more["deposited"], 1)
        belt = self._rows_by_label()["Belt"]
        self.assertEqual(belt.page_index, 0)
        self.assertNotEqual((belt.layout_x, belt.layout_y), (0, 0))

    def test_gear_overflowing_the_expedition_page_continues_on_a_named_page(self):
        ...
        # The next expedition starts on its own page after the spill-over.
        other = editor.op_vault_ingest({"expedition_id": "exp_two", "records": [
            spool_record(1, 3, "Cannon", {"n": 2.0, "b": 0.0, "a": 1.0, "j": 14.0, "c": 0.0}),
        ]})
        self.assertEqual(other["collections"]["farm"]["pageIndex"], 2)
        self.assertEqual(
            [page.name for page in store.list_stash_pages(farm_id)],
            [PAGE_NAME, PAGE_NAME + " (2)", "exp_two \u00b7 2026-09-17"],
        )
