...
async function openItemForge(preset){
  ...
  app.innerHTML=`<header class="f-heading"><div><div class="f-eyebrow">Your item workshop</div><h2>Item Forge</h2><p>Choose an item. Shape its properties. Make it yours.</p></div><span class="f-badge" id="ifdraftbadge">New workshop</span></header>
    ...
    <div class="f-workspace" id="ifworkspace" hidden><div class="f-main"><div class="f-card f-base" id="ifselected"></div>
      ...
      <div id="ifpanel-effects" data-edit-panel="effects" role="tabpanel" aria-labelledby="iftab-effects" hidden><div class="f-section-head"><div><h3>A special ability</h3><p>Optional. Choose one behavior for this item.</p></div></div><select id="ifmech" aria-label="Special mechanic" hidden><option value="">None</option><option value="headhunter">Headhunter</option><option value="tyrant">Tyrant's Crown</option><option value="miner">Miner's Helmet</option></select>${[['','No extra ability','Use only the properties you selected.'],['headhunter','Headhunter','Defeating a rare monster grants its affixes as temporary buffs.'],['tyrant',"Tyrant’s Crown",'Nearby monsters become rare more often. Rare monsters gain one extra affix.'],['miner',"Miner's Helmet",'Helmet only. Exactly 4x mining ore while worn, and Vein Resonance: each finished dig also digs the two nearest veins you could mine. Requires ForgePact 1.4.5 or later.']].map(([value,name,desc])=>`<label class="f-mechanic"><input type="radio" name="forge-mechanic" value="${value}" ${!value?'checked':''}><span><b>${esc(name)}${value==='headhunter'?'<em>Preview</em>':''}</b><small>${esc(desc)}</small></span></label>`).join('')}<p class="f-preview-note">Headhunter and Tyrant require their ForgePact switches. Miner's Helmet requires ForgePact 1.4.5 or later and works while worn.</p></div></section>
