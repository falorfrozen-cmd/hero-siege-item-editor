...
const ITEMFORGE_TIERS=[['','Original tier'],['1','C'],['2','B'],['3','A'],['4','S'],['5','SS']];
...
function openForgeSignaturePicker(onPick){
  const d=forgeDialog('Forge a signature item','Ready-made items with their ForgePact mechanics. Pick one, adjust anything you like, then review and save it.',`<div class="f-signature-list" data-signature-list><p role="status">Loading…</p></div>`,'');
  (async()=>{
...
async function openItemForge(preset){
  ...
  app.innerHTML=`<header class="f-heading"><div><div class="f-eyebrow">Your item workshop</div><h2>Item Forge</h2><p>Choose an item. Shape its properties. Make it yours.</p></div><span class="f-badge" id="ifdraftbadge">New workshop</span></header>
    ...
    <div class="f-workspace" id="ifworkspace" hidden><div class="f-main"><div class="f-card f-base" id="ifselected"></div>
      ...
      <div id="ifpanel-appearance" data-edit-panel="appearance" role="tabpanel" aria-labelledby="iftab-appearance" hidden><div class="f-section-head"><div><h3>A name and a story</h3><p>Optional. Leave a field empty to keep the original appearance.</p></div></div><div class="f-form"><label><span class="f-label-head">Custom name <small id="ifnamecount">0 / 48</small></span><input id="ifname" maxlength="48" placeholder="Keep original item name" autocomplete="off"></label><label>Rarity color<select id="ifrarity">${ITEMFORGE_RARITIES.map(([value,label])=>`<option value="${value}">${esc(label)}</option>`).join('')}</select></label><label>Tier<select id="iftier">${ITEMFORGE_TIERS.map(([value,label])=>`<option value="${value}">${esc(label)}</option>`).join('')}</select><small>The Tier letter in the tooltip; loot filters use it. SS for the strongest items.</small></label><label class="wide"><span class="f-label-head">Special text <small id="ifaffixcount">0 / 240</small></span><textarea id="ifaffix" maxlength="240" rows="3" placeholder="A short golden inscription…"></textarea><small>Up to 3 lines above the stats. This text does not add an effect.</small></label><label class="wide"><span class="f-label-head">Lore <small id="iflorecount">0 / 1000</small></span><textarea id="iflore" maxlength="1000" rows="3" placeholder="Tell the story behind your item…"></textarea><small>Flavor text below the stats.</small></label></div></div>
      <div id="ifpanel-effects" data-edit-panel="effects" role="tabpanel" aria-labelledby="iftab-effects" hidden><div class="f-section-head"><div><h3>A special ability</h3><p>Optional. Choose one behavior for this item.</p></div></div><select id="ifmech" aria-label="Special mechanic" hidden><option value="">None</option><option value="headhunter">Headhunter</option><option value="tyrant">Tyrant's Crown</option><option value="miner">Miner's Helmet (prototype)</option></select>${[['','No extra ability','Use only the properties you selected.'],['headhunter','Headhunter','Defeating a rare monster grants its affixes as temporary buffs.'],['tyrant',"Tyrant’s Crown",'Nearby monsters become rare more often. Rare monsters gain one extra affix.'],['miner',"Miner's Helmet",'Helmet only. Exactly 4x mining ore while worn and a golden pulse. Requires the Miner prototype plugin. Nearby-vein harvesting is not enabled yet.']].map(([value,name,desc])=>`<label class="f-mechanic"><input type="radio" name="forge-mechanic" value="${value}" ${!value?'checked':''}><span><b>${esc(name)}${value==='headhunter'?'<em>Preview</em>':''}</b><small>${esc(desc)}</small></span></label>`).join('')}<p class="f-preview-note">Headhunter and Tyrant require their ForgePact switches. Miner requires the local Miner prototype plugin and a worn helmet.</p></div></section>
  ...
  const identity=()=>({name:q('ifname').value.trim(),rarity:q('ifrarity').value,tier:q('iftier').value,lore:q('iflore').value.trim(),affix:q('ifaffix').value.trim(),mechanic:q('ifmech').value});
  ...
  function fill(cfg){
    editor.setConfiguration(cfg);q('ifname').value=cfg?.name||'';q('ifrarity').value=cfg?.rarity!=null?String(cfg.rarity):'';q('iftier').value=cfg?.tier!=null?String(cfg.tier):'';q('iflore').value=cfg?.lore||'';q('ifaffix').value=cfg?.affix||'';q('ifmech').value=cfg?.mechanic||'';app.querySelectorAll('[name=forge-mechanic]').forEach(radio=>radio.checked=radio.value===q('ifmech').value);
  ...
  ['ifname','ifrarity','iftier','iflore','ifaffix','iftab'].forEach(id=>{const input=q(id);input.oninput=()=>{message('');update()};if(input.tagName==='SELECT')input.onchange=input.oninput});
  ...
  q('ifgo').onclick=async()=>{
    ...
    try{
      ...
      const body={action:'apply',...reference,stats:state.stats,presetIds:state.presetIds,excludedKeys:state.excludedKeys,keepNative:state.keepNative,name:name||null,lore:lore||null,affix:affix||null,rarity:rarity===''?null:+rarity,tier:tier===''?null:+tier,mechanic:q('ifmech').value||null};
